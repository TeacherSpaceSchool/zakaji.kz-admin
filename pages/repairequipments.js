...
const RepairEquipments = React.memo((props) => {
    ...
    return (
        <App cityShow checkPagination={checkPagination} pageName='Ремонт'>
            <Head>
                <title>Ремонт</title>
                ...
                <meta property="og:url" content={`${urlMain}/repairequipments`} />
                <link rel='canonical' href={`${urlMain}/repairequipments`}/>
            </Head>
            <div className='count'>
                    ...
                    if(idx<pagination)
                        return(
                            <LazyLoad scrollContainer={'.App-body'} key={element._id} height={height} offset={[height, 0]} debounce={0} once={true}  placeholder={<CardOrganizationPlaceholder height={height}/>}>
                                <Link href='/repairequipments/[id]' as={`/repairequipments/${element._id}`}>
                                    <a>
