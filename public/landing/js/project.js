...
t431_parseData = function(t431__data) {
    if (t431__data!="" && typeof t431__data!='undefined')
    {
        ...
        for (var i=0; i<t431__data.length; i++)
        {
            ...
            if (t431__data[i] == ";" && !(t431__data.slice(i-4, i) == "&lt;" || t431__data.slice(i-4, i) == "&gt;" || t431__data.slice(i-5, i) == "&amp;" || t431__data.slice(i-6, i) == "&nbsp;")) {
...
function t754__alignButtons_init(recid) {
    ...
    if (rec.find('[data-buttons-v-align]')[0]) {
        try {
            ...
            $('.t754').bind('displayChanged', function() {
                t754__alignButtons(recid);
            });
...
function t786__alignButtons_init(recid) {
    ...
    if (rec.find('[data-buttons-v-align]')[0]) {
        try {
            ...
            $('.786').bind('displayChanged', function() {
                t786__alignButtons(recid);
            });
...
function t803_init(recid){
    ...
    $.each($('[field]'), function(){
        fix = $(this).html().replace(/(?!%%)[A-zÐ-ÑÑ‘0-9_-]*(?=%%)/ig, '<span data-replace-key="$&"></span>');
...
function t881_addWebinar(rec) {
    ...
    if ($(window).scrollTop() > blockTop - windowHeight - scrollDiffHeigth) {
        if (wrap.attr('data-webinar-load') == 'false') {
            ...
            if (comments.hasClass('t881__webinar-comments-youtube')) {
                if (!window.isMobile) {
                    comments.prepend('<iframe src="https://www.youtube.com/live_chat?v=' + webinarId + '&embed_domain=' + window.location.hostname + '" width="100%" height="100%"></iframe>');
                } else {
...
function t899_init(recid) {
    ...
    $('.t899').bind('displayChanged', function() {
        t899_setMinHeight(recid);
    });
...
function t899_getWrappers(rec) {
    var el = document.querySelector('#rec' + rec);
    if (!el) {
        return;
    }
    var wrappers = el.querySelectorAll('.t899__wrapper');

    return wrappers;
}

function t899_setMinHeight(rec) {
    ...
    var wrappers = t899_getWrappers(rec);
    if (wrappers) {
        forEach(wrappers, function(index, wrapper) {
            var symbol = wrapper.querySelector('.t899__symbol');
            if (symbol && symbol.offsetHeight > wrapper.offsetHeight) {
                wrapper.style.minHeight = symbol.offsetHeight + 'px';
            }
        });
    }
}
...
function t923_init(recid){
    ...
    $(window).bind('resize', t_throttle(function(){t923_unifyHeights(recid)}, 200));

    $(".t923").bind("displayChanged",function(){
        setTimeout(function() {
            t923_unifyHeights(recid);
            t_slds_updateSlider(recid);
        });
...
function t923_unifyHeights(recid){
    ...
    var t923_el = $('#rec'+recid),
        t923_cols = t923_el.find(".t923__content");

    var t923_maxHeight = 0;

    t923_cols.each(function() {
        var t923_curText = $(this).find(".t923__textwrapper"),
            t923_curBtns = $(this).find(".t923__btn-wrapper, .t923__btntext-wrapper"),
            t923_itemHeight = t923_curText.outerHeight() + t923_curBtns.outerHeight();

        if ( t923_itemHeight > t923_maxHeight ) { t923_maxHeight = t923_itemHeight; }
    });
